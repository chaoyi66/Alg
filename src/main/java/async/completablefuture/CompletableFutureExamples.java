...
//            allOfAsyncExample();
	...
	static void acceptEitherExample() {
		String original = "Message";
		StringBuilder result = new StringBuilder();
		CompletableFuture<String> otherFuture = CompletableFuture.completedFuture(original)
				.thenApplyAsync(s -> delayedLowerCase(s));
		CompletableFuture<String> oneFuture = CompletableFuture.completedFuture(original)
				.thenApplyAsync(s -> delayedUpperCase(s));
		Consumer<String> consumer = s -> result.append(s).append("acceptEither");
		CompletableFuture<Void> cf = oneFuture.acceptEither(otherFuture, consumer);
		cf.join();
	...
	static void runAfterBothExample() {
		...
		CompletableFuture.completedFuture(original).thenApply(String::toUpperCase).thenAcceptBoth(
				...
				(s1, s2) -> result.append(s1 + s2 + "done"));
	...
	private static String delayedUpperCase(String s) {
		randomSleep();
		System.out.println("!!!" + Thread.currentThread().getName() + ": delayedUpperCase");
		return s.toUpperCase();
	}

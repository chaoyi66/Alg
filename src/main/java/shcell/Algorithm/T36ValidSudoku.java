...
public class T36ValidSudoku {
	...
	public static boolean isValidSudoku(char[][] board) {
		...
		boolean[][] colValid = new boolean[len][], rowValid = new boolean[len][],
				squreValid = new boolean[len][];
		for (int i = 0; i < len; i++) {
			colValid[i] = new boolean[len];
			rowValid[i] = new boolean[len];
			squreValid[i] = new boolean[len];
		}
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < len; j++) {
				if (board[i][j] == '.')
